...
parser.add_argument(
    "-t",
    "--time-delay",
    help="Time delay for availability polling (in seconds), default: 2s",
    type=float,
    required=False,
)
parser.add_argument("-k", "--key", help="API key generated by Twickets", required=True)
parser.add_argument(
    "-P", "--prowl", help="Optional API key for Prowl notifications", required=False
)
...
def get_ticket_avail(inventory_id, seats):
    url = (
        "https://www.twickets.live/services/inventory/"
        + str(inventory_id)
        + "?api_key="
        + api_key
        + "&qty="
    ...
    cookies = {
        "locale": "en_GB",
        "territory": "GB",
        "clientId": "3442ef49-050c-41d8-9ebd-c21408fe5c18",
    }
...
def check_event_avail(event_id):
    url = (
        "https://www.twickets.live/services/g2/inventory/listings/"
        + str(event_id)
        + "?api_key="
        + api_key
    )
...
if __name__ == "__main__":
    ...
    logging.basicConfig(
        format=format, level=logging.INFO, datefmt="%H:%M:%S", stream=sys.stdout
    )
    if args.prowl is not None:
    ...
    try:
        ...
        p.notify(
            event="Twickets",
            description="Twicket loop started",
            priority=0,
            url="https://www.twickets.live/event/1799517104504836096",
            appName="twicket-get",
        )
                    ...
                    if ticket["available"] and int(current["seats"]) < 4:
                        ...
                        if int(ticket["deliveryPlan"][0]["deliveryMethod"]):
                            if int(ticket["deliveryPlan"][0]["deliveryMethod"]) == 1:
                                logging.warning(
                                    "Main loop   : [Skipped] Delivery method was "
                                    + ticket["deliveryPlan"][0]["title"]
                                )
                                continue
                        hold_details = request_hold(
                            ticket["block"]["blockId"],
                            current["seats"],
                            authorization_token,
                        )
                        if hold_details:
                            print(
                                "Hold details: "
                                + hold_details["holdReference"]
                                + ": expires "
                                + hold_details["expires"]
                            )
                            prebook_result = prebook(
                                ticket["block"]["blockId"],
                                hold_details["holdReference"],
                                current["seats"],
                                authorization_token,
                            ...
                            if prebook_result:
                                p.notify(
                                    ...
                                    + str(current["price"]),
                                    priority=0,
                                    url=prebook_result["redirectUrl"],
                                    appName="twicket-get",
                                )
                                logging.info(
                                    "Main loop   : Prebooking success: https://www.twickets.live/block/"
                                    + str(ticket["block"]["blockId"])
                                    + ","
                                    + str(current["seats"])
                                    + "\n"
                                ...
                                logging.error(
                                    "Main loop   : Prebooking failed: https://www.twickets.live/block/"
                                    + str(ticket["block"]["blockId"])
                                    + ","
                                    + str(current["seats"])
                                )
                                p.notify(
                                    ...
                                    + str(current["price"]),
                                    priority=0,
                                    url="https://www.twickets.live/block/"
                                    + str(ticket["block"]["blockId"])
                                    + ","
                                    + str(current["seats"]),
                                    appName="twicket-get",
